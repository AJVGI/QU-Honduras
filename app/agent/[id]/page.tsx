    ...
    <div className="space-y-6 max-w-5xl">
      ...
      {tab === 'coaching' && (
        <div className="space-y-4">
          <div className="bg-[#1A1A2E] border border-[#7B2D8B]/20 rounded-xl p-5 glow-panel">
            <h3 className="text-sm font-semibold text-[#E91E8C] mb-4">💡 Top Coaching Tips (from QA grading)</h3>
            {coachingTips.length > 0 ? (
              <div className="space-y-3">
                {coachingTips.map(([tip, count], i) => (
                  <div key={i} className="flex gap-3 bg-blue-900/10 border border-blue-500/20 rounded-lg px-3 py-2">
                    <span className="text-[#E91E8C] font-bold text-sm flex-shrink-0">×{count}</span>
                    <p className="text-sm text-slate-300">{tip}</p>
                  </div>
                ))}
              </div>
            ) : <p className="text-slate-500 text-sm">No coaching tips yet — run pipeline to grade more tickets.</p>}
          </div>
        </div>
      )}
    </div>
